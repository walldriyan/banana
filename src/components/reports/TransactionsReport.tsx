// src/components/reports/TransactionsReport.tsx
import React from 'react';
import { useLanguage } from '@/context/LanguageContext';
import type { Transaction, Customer } from '@prisma/client';
import { format } from 'date-fns';

type CompletedTransaction = Transaction & { customer: Customer };
...
export const TransactionsReport: React.FC<TransactionsReportProps> = ({ data }) => {
  const { transactions, dateRange } = data;
  const { t, language } = useLanguage();
  const grandTotal = transactions.reduce((sum, tx) => sum + tx.finalTotal, 0);

  return (
    <div className="report-container p-4 bg-white text-sm text-black">
        <header className="text-center mb-6">
            <h1 className="text-xl font-bold text-gray-800">Completed Transactions Report</h1>
        ...
        <table className="w-full text-left border-collapse">
            <thead>
                <tr className="bg-gray-100">
                    ...
                    <th className="p-2 border-b-2 border-gray-300 font-semibold text-black">Payment Status</th>
                    <th className="p-2 border-b-2 border-gray-300 font-semibold text-black text-right">Final Total</th>
                </tr>
            </thead>
            <tbody>
                {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-gray-50">
                        <td className="p-2 border-b border-gray-200">{format(new Date(tx.transactionDate), 'yyyy-MM-dd HH:mm')}</td>
                        ...
                        <td className="p-2 border-b border-gray-200 capitalize">{tx.paymentStatus}</td>
                        <td className="p-2 border-b border-gray-200 text-right font-semibold">Rs. {tx.finalTotal.toFixed(2)}</td>
                    </tr>
                ))}
            </tbody>
             <tfoot>
                <tr className="bg-gray-100 font-bold">
                    ...
                    <td className="p-2 text-right">Rs. {grandTotal.toFixed(2)}</td>
                </tr>
            </tfoot>
        </table>
